...
public:
	...
	TeapotSpring(UINT width = SUB_TEXTUREWIDTH, UINT height = SUB_TEXTUREHEIGHT)
	{
		...
		m_CBallInOne.vSpherePos = XMFLOAT4(1.4,-1.5,0.1,1);
		...
		m_CBallInOne.fGroundHeight = -3.0f;
	}
	...
	HRESULT CreateResource(ID3D11Device* pd3dDevice)
	{
		...
		wstring filename=L"TeapotSpring.fx";
		...
		V_RETURN(CompileShaderFromFile(L"TeapotSpring.fx","RenderGS","gs_5_0",&pGSBlob));
		V_RETURN(pd3dDevice->CreateGeometryShader(pGSBlob->GetBufferPointer(),pGSBlob->GetBufferSize(),NULL,&m_pRenderParticleGS));
		V_RETURN(CompileShaderFromFile(L"TeapotSpring.fx","GroundGS","gs_5_0",&pGSBlob));
		V_RETURN(pd3dDevice->CreateGeometryShader(pGSBlob->GetBufferPointer(),pGSBlob->GetBufferSize(),NULL,&m_pGroundGS));
		V_RETURN(CompileShaderFromFile(L"TeapotSpring.fx","AdvanceGS","gs_5_0",&pGSBlob));
		...
		V_RETURN(CompileShaderFromFile(L"TeapotSpring.fx","RenderPS","ps_5_0",&pPSBlob));
		V_RETURN(pd3dDevice->CreatePixelShader(pPSBlob->GetBufferPointer(),pPSBlob->GetBufferSize(),NULL,&m_pRenderParticlePS));
		
		V_RETURN(CompileShaderFromFile(L"TeapotSpring.fx","MeshPS","ps_5_0",&pPSBlob));
		...
		bd.CPUAccessFlags = 0    ;
		bd.ByteWidth = sizeof(TS_ConstBuffer);
	...
	void Render(ID3D11DeviceContext* pd3dImmediateContext, double fTime, float fElapsedTime)
	{
		...
		m_CBallInOne.mTeapot = XMMatrixTranspose(m_World*XMMatrixTranslation(-2,0,0));
